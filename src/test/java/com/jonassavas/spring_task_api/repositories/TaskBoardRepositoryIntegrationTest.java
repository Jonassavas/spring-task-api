...
public class TaskBoardRepositoryIntegrationTest {
    ...
    public void testThatTaskBoardCanBeCreatedAndRecalled() {
        TaskBoardEntity testTaskBoard =
                TestTaskBoardData.createTestTaskBoardEntityA(user);

        TaskBoardEntity saved = underTest.saveAndFlush(testTaskBoard);

        Optional<TaskBoardEntity> result =
                underTest.findById(saved.getId());

        assertThat(result).isPresent();
        assertThat(result.get().getTaskBoardName())
                .isEqualTo(testTaskBoard.getTaskBoardName());
    }

    @Test
    public void testThatMultipleTaskBoardsCanBeCreatedAndRecalled() {
        TaskBoardEntity boardA = underTest.saveAndFlush(
                TestTaskBoardData.createTestTaskBoardEntityA(user)
        );
        TaskBoardEntity boardB = underTest.saveAndFlush(
                TestTaskBoardData.createTestTaskBoardEntityB(user)
        );
        TaskBoardEntity boardC = underTest.saveAndFlush(
                TestTaskBoardData.createTestTaskBoardEntityC(user)
        );
    ...
    public void testThatTaskBoardCanBeUpdated() {
        TaskBoardEntity board = underTest.saveAndFlush(
                TestTaskBoardData.createTestTaskBoardEntityA(user)
        );
        ...
        Optional<TaskBoardEntity> result =
                underTest.findById(board.getId());
    ...
    public void testFindByIdAndOwnerUsername() {
        ...
        var result = underTest.findByIdAndOwnerUsername(
                board.getId(),
                user.getUsername()
        );
        ...
        assertThat(result).isPresent();
        assertThat(result.get().getTaskBoardName())
                .isEqualTo(board.getTaskBoardName());
    }
